    ...
    <>
      ...
      >
        ...
        >
          ...
          {!forceHamburger ? (
            <>
              {/* Navigation Items - Updated with better spacing */}
              <div className="nav-items-section flex items-center flex-1 justify-center mx-2 sm:mx-4">
                <div className="flex items-center gap-2 sm:gap-3 md:gap-4 lg:gap-6 xl:gap-8 text-gray-700 font-medium">
                  {navItems.map((item, index) => (
                    <Link
                      key={item.path}
                      to={item.path}
                      className={`flex items-center gap-1 px-2 sm:px-3 md:px-4 lg:px-5 py-1.5 md:py-2 rounded-lg transition-all duration-300 relative text-xs sm:text-sm flex-shrink-0 ${
