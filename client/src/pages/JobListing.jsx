...
const navigate = useNavigate();
  ...
  }, [jobs, selectedCategory, selectedLocation, selectedChannel, selectedJobCategory, searchFilter]);
            ...
            <>
              ...
              <div className="mb-8">
                <div className="flex justify-between items-center mb-4">
                  <h4 className="font-bold text-lg text-gray-800">Designation</h4>
                ...
                <ul className="space-y-3">
                  {Jobdesignation.slice(0, showAllDesignations ? Jobdesignation.length : 10).map((designation, index) => (
                    <motion.li 
                      key={index} 
                    ...
                    >
                      <input
                        ...
                        type="checkbox"
                        onChange={() => handleCategoryChange(designation)}
                        checked={selectedCategory.includes(designation)}
                ...
                {Jobdesignation.length > 10 && (
                  <button
                    onClick={() => setShowAllDesignations(!showAllDesignations)}
                    className="mt-2 text-sm text-primary hover:underline"
                  >
                    {showAllDesignations ? 'Show less' : `Show all (${Jobdesignation.length})`}
                  </button>
                )}
              </div>
              ...
              <div className="mb-8">
                ...
                <ul className="space-y-3">
                  {JobChannels.slice(0, showAllChannels ? JobChannels.length : 8).map((channel, index) => (
                    <motion.li 
                      key={index} 
                      className="flex items-center"
                      whileHover={{ x: 3 }}
                ...
                {JobChannels.length > 8 && (
                  <button
                    onClick={() => setShowAllChannels(!showAllChannels)}
                    className="mt-2 text-sm text-primary hover:underline"
                  >
                    {showAllChannels ? 'Show less' : `Show all (${JobChannels.length})`}
                  </button>
                )}
              </div>
              ...
              <div className="mb-4">
                ...
                <ul className="space-y-3">
                  {JobCategories.slice(0, showAllJobCategories ? JobCategories.length : 8).map((jobCategory, index) => (
                    <motion.li 
                      key={index} 
                      className="flex items-center"
                      whileHover={{ x: 3 }}
                    >
                      <input
                        className="h-4 w-4 text-primary rounded focus:ring-primary border-gray-300"
                        type="checkbox"
                        onChange={() => handleJobCategoryChange(jobCategory)}
                        checked={selectedJobCategory.includes(jobCategory)}
                ...
                {JobCategories.length > 8 && (
                  <button
                    onClick={() => setShowAllJobCategories(!showAllJobCategories)}
                    className="mt-2 text-sm text-primary hover:underline"
                  >
                    {showAllJobCategories ? 'Show less' : `Show all (${JobCategories.length})`}
                  </button>
                )}
              </div>
            </>
      ...
      <section className="w-full lg:w-3/4 pl-0 lg:pl-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
          <p className="text-gray-600 mb-2 sm:mb-0">
            ...
            {(selectedCategory.length > 0 || selectedLocation.length > 0 || selectedChannel.length > 0 || selectedJobCategory.length > 0) && (
